          ...
          <div class="panel-body p-b16">
            <table class="table hover striped theme-dft pad-table ">
              ...
              <tr v-for="item in employees">
                ...
                <td ><div ><m-btn v-if="isShow('audit_employee')" @click.native="company_reject(item.id)">拒绝</m-btn><m-btn v-if="isShow('audit_employee')" @click.native="company_accept(item.id)">允许</m-btn><m-btn v-if="isShow('set_employee_permission')" @click.native="changeUserTemp(item.uid)">权限</m-btn><m-btn v-else  @click.native="lookUserTemp(item.uid)">查看</m-btn><m-btn v-if="isShow('dismiss_employee')"  @click.native="company_dismission(item.id)">解除</m-btn></div></td>
              </tr>
              </tbody>
            </table>
          </div>
