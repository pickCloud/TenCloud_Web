          ...
          <div class="panel-body p-b16">
            <table class="table hover striped theme-dft pad-table ">
              ...
              <tr v-for="item in employees">
                ...
                <td ><div ><span v-if="item.status===5"><m-btn v-if="isShow('audit_employee')" @click.native="company_reject(item.id)">拒绝</m-btn><m-btn v-if="isShow('audit_employee')" @click.native="company_accept(item.id)">允许</m-btn></span><span v-if="item.status===4 || item.status===3"><m-btn v-if="isAdmin" @click.native="changeUserTemp(item.uid)">权限</m-btn><m-btn v-else  @click.native="lookUserTemp(item.uid)">查看</m-btn><m-btn v-if="isAdmin"  @click.native="company_dismission(item.id)">解除</m-btn></span></div></td>
              </tr>
              </tbody>
            </table>
          </div>
